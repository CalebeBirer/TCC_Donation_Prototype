...
1.  **Governança:** Definição de endereços aprovadores e quórum mínimo no momento da implantação (`constructor`).
2.  **Criação de Pedidos:** Apenas aprovadores podem registrar novas demandas de doação (`criarPedido`), informando destino, valor e descrição.
3.  **Sistema de Quórum:** Registro individual de aprovações (`aprovarPedido`), impedindo que um mesmo endereço vote duas vezes.
4.  **Execução Automatizada:** A transferência dos fundos só é liberada (`executarPedido`) após o quórum ser atingido e a segurança financeira ser validada.
5.  **Auditabilidade:** Emissão de eventos (`emit`) para rastreio permanente "on-chain" e função de consulta pública (`getPedido`).
...
1.  **Criar Pedido:** Use a função `criarPedido` preenchendo o endereço de destino e o valor em "Wei".
2.  **Aprovar:** Com uma conta autorizada, chame `aprovarPedido` informando o ID gerado.
3.  **Consultar:** A qualquer momento, use `getPedido` para verificar o status e quantas aprovações foram coletadas.
4.  **Executar:** Após atingir o quórum, chame `executarPedido` para realizar a transferência automática.
...
| Função | Custo de Execução (Gas) |
| :--- | :--- |
| Implantação | 1.406.910 |
| Criar Pedido | 101.407 |
| 1ª Aprovação | 54.542 |
| 2ª Aprovação | 37.442 (Redução de ~31%) |

## ⚖️ Licença
Este projeto foi desenvolvido como parte de um Trabalho de Conclusão de Curso (TCC) do **MBA USP/Esalq** em Engenharia de Software. O código é distribuído sob a licença **MIT**.
